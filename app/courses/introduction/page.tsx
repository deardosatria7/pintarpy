...
export default async function IntroductionPage({
  ...
  // Mengambil parameter step dari URL, default ke "1" jika tidak ada
  const currentStep = Number.parseInt((await searchParams).step || "1");
    ...
    >
      <div className="flex flex-col w-full space-y-6 p-4 md:p-8">
              ...
              <div className="px-4 py-2 overflow-x-auto">
                <TabsList className="grid grid-flow-col auto-cols-max gap-2">
                  {lessonSteps.map((step) => (
                    <Link
                      key={step.id}
                      href={`/courses/introduction?step=${step.id}`}
                      className={`
        ...
        <div className="flex justify-between mt-4">
          ...
          >
            {currentStep > 1 ? (
              <Link href={`/courses/introduction?step=${currentStep - 1}`}>
          ...
          {currentStep < lessonSteps.length ? (
            <Button
              className="bg-purple-700 hover:bg-purple-800 dark:bg-purple-800 dark:text-white dark:hover:bg-purple-700 gap-1"
              asChild
            >
              <Link href={`/courses/introduction?step=${currentStep + 1}`}>
                Selanjutnya
            ...
            <Button
              ...
              asChild
            >
              <Link href="/courses/variabel-dan-tipe-data">
                Materi Berikutnya
                <ChevronRight className="h-4 w-4" />
              </Link>
            </Button>
